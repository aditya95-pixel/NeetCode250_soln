    ...
    bool isMatch(string s, string p) {
        ...
        for(int i=1;i<=s.size();i++){
            for(int j=1;j<=p.size();j++){
                ...
                dp[i][j]=dp[i-1][j-1];
                else if(p[j-1]=='*' && j>=2 && (p[j-2]==s[i-1] || p[j-2]=='.'))
                dp[i][j]=dp[i-1][j];
                if(p[j-1]=='*' && j>=2)
...
## 18 Number of Alternating XOR Partitions
...
public:
    int alternatingXOR(vector<int>& nums, int target1, int target2) {
        unordered_map<int,unordered_map<int,int>>xors;
        vector<vector<int>>dp(nums.size(),(vector<int>(2,0)));
        xors[target2][0]++;
        int xoro=0;
        for(int i=0;i<nums.size();i++){
            xoro^=nums[i];
            dp[i][0]=(dp[i][0]+xors[target2][xoro^target1])%MOD;
            dp[i][1]=(dp[i][1]+xors[target1][xoro^target2])%MOD;
            xors[target1][xoro]=(xors[target1][xoro]+dp[i][0])%MOD;
            xors[target2][xoro]=(xors[target2][xoro]+dp[i][1])%MOD;
        }
        long long ans=(dp.back()[0]+dp.back()[1])%MOD;
        return ans;
    }
};
```
